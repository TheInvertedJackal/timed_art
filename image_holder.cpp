...
ImageHolder::ImageHolder(SDL_Renderer* rend,string path, double x, double y): 
    _rend{rend}, _path{path}, _img_in_use{false}, _x_fit{x}, _y_fit{y}{}
...
ImagePool::ImagePool():
     _useable_count{0}, _used_pointer{0}, _total_count{0}, _used_end{0}, _used_count{0}{}
...
ImageHolder* ImagePool::getFreshImage(){
    ...
    //Random value bla 0 to end
    int rv = 0; //Totaly random
    ...
    _used_images[_used_pointer];
    _used_pointer = (_used_pointer + 1) % MAX_IMAGES;
    ++_used_count;
    if(((double) _used_count) / _total_count >= .6){
        --_used_count;
